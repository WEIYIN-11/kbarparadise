...
## 標的（動態組成，約 74 檔，清單見 `data/symbols.json`）
- **期指**（固定）：NQ 那斯達克100期貨、TXF 台指期（1H 用 ^TWII 加權指數替代）
- **加密貨幣**：Binance USDT 現貨「近 30 天成交額」**前 20 名**（每次 `npm run fetch` 重新排名）＋固定保底 BTC/ETH/SOL/BNB/XRP/DOGE
- **美股**：候選池內「近 30 天成交量」**前 20 名**＋固定保底既有 11 檔
- **台股**：候選池內「近 30 天成交量」**前 20 名**＋固定保底既有 10 檔
